...
bool nav2_msgs__msg__tracking_feedback__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("nav2_msgs.msg._tracking_feedback.TrackingFeedback", full_classname_dest, 49) == 0);
  }
  ...
  {  // tracking_error
    PyObject * field = PyObject_GetAttrString(_pymsg, "tracking_error");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->tracking_error = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // current_path_index
...
PyObject * nav2_msgs__msg__tracking_feedback__convert_to_py(void * raw_ros_message)
{
  ...
  {  // tracking_error
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->tracking_error);
    {
      int rc = PyObject_SetAttrString(_pymessage, "tracking_error", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
