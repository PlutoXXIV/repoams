...
class FollowPath_Goal(metaclass=Metaclass_FollowPath_Goal):
    ...
    def __init__(self, **kwargs):
        ...
        self.progress_checker_id = kwargs.get('progress_checker_id', str())
...
class Metaclass_FollowPath_Result(type):
    ...
    def __prepare__(cls, name, bases, **kwargs):
        ...
        return {
            ...
            'INVALID_CONTROLLER': cls.__constants['INVALID_CONTROLLER'],
            'TF_ERROR': cls.__constants['TF_ERROR'],
            'INVALID_PATH': cls.__constants['INVALID_PATH'],
            'PATIENCE_EXCEEDED': cls.__constants['PATIENCE_EXCEEDED'],
            'FAILED_TO_MAKE_PROGRESS': cls.__constants['FAILED_TO_MAKE_PROGRESS'],
            'NO_VALID_CONTROL': cls.__constants['NO_VALID_CONTROL'],
            'CONTROLLER_TIMED_OUT': cls.__constants['CONTROLLER_TIMED_OUT'],
        }
...
class Metaclass_FollowPath_Feedback(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            if TrackingFeedback.__class__._TYPE_SUPPORT is None:
                TrackingFeedback.__class__.__import_type_support__()
    ...
    def __init__(self, **kwargs):
        ...
        if self._check_fields:
            assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
                ...
                ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        from nav2_msgs.msg import TrackingFeedback
        self.tracking_feedback = kwargs.get('tracking_feedback', TrackingFeedback())
...
class FollowPath_SendGoal(metaclass=Metaclass_FollowPath_SendGoal):
    ...
    from nav2_msgs.action._follow_path import FollowPath_SendGoal_Event as Event
...
class FollowPath_GetResult(metaclass=Metaclass_FollowPath_GetResult):
    ...
    from nav2_msgs.action._follow_path import FollowPath_GetResult_Event as Event
